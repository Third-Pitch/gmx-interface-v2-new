...
export function EmSwapBox(p: Props) {
  ...
  const currentEMIcon = getIcon(chainId, "em");
      ...
      >
        <div className={cx("EmSwapBox-form-layout", { reverse: isWithdrawal })}>
          ...
          <BuyInputSection
            ...
            onClickMax={() => {
              if (marketToken?.balance) {
                const formattedEMBalance = formatAmountFree(marketToken.balance, marketToken.decimals);
                const finalEMBalance = isMetamaskMobile
                  ? limitDecimals(formattedEMBalance, MAX_METAMASK_MOBILE_DECIMALS)
                  : formattedEMBalance;
                setMarketTokenInputValue(finalEMBalance);
                setFocusedInput("market");
              }
            }}
          >
            <div className="selected-token">
              <img className="mr-xs" width={20} src={currentEMIcon} alt="EM Token" />
              EM
            </div>
          </BuyInputSection>
        </div>
