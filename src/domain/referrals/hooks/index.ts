...
import { BASE, SUPPORTED_CHAIN_IDS } from "config/chains";
...
export function useUserCodesOnAllChain(account) {
  ...
  useEffect(() => {
    async function main() {
      const [baseCodes] = await Promise.all(
      ...
      const [codeOwnersOnBase = []] = await Promise.all([
        getCodeOwnersData(BASE, account, baseCodes),
      ]);

      setData({
        [BASE]: codeOwnersOnBase.reduce((acc, cv) => {
          acc[cv.code] = cv;
          return acc;
        }, {} as any),
